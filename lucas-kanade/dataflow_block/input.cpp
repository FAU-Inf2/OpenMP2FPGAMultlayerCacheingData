...
void knp(A &im1, B &im2, float *out, int width, int height, int window_size) {
loop_main:
...
gen_shifted_img_pair(size_t width, size_t height, int shiftx, int shifty) {
  unsigned char *imga = new unsigned char[SIZESQ];
  unsigned char *imgb = new unsigned char[SIZESQ];
